...
try:
    ...
    logging.warning("Zoom control not available. Install: pip install pynput")
...
class ZoomController:
    ...
    def __init__(self, app=None):
        ...
        self.app = app
        self.mouse = pynput_mouse.Controller() if self.zoom_available else None
    ...
    def zoom_out(self, steps: Optional[int] = None) -> bool:
        ...
        try:
            for i in range(steps):
                # Scroll down on macOS (negative = zoom out)
    ...
    def zoom_in(self, steps: Optional[int] = None) -> bool:
        ...
        try:
            for i in range(steps):
                # Scroll up on macOS (positive = zoom in)
                self.mouse.scroll(0, 1)
                time.sleep(self.zoom_settings["step_delay"])
